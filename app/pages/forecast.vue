...
async function fetchForecast() {
  loading.value = true
  try {
    const response = await $fetch(`/api/forecast/${hall.value.value}`)
...
async function fetchWeekForecast() {
  ...
  try {
    const response = await $fetch(`/api/forecast-week/${hall.value.value}`)
...
async function fetchDateForecast() {
  ...
  try {
    const response: any = await $fetch(`/api/forecast-date/${hall.value.value}/${dateStr}`)
...
.chart-card:hover {
  ...
  box-shadow: 0 20px 40px rgba(255, 179, 0, 0.1) !important;
}
